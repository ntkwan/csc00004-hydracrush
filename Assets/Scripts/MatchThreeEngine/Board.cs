...
namespace MatchThreeEngine
{
	public sealed class Board : MonoBehaviour
	{
		...
		[SerializeField] private AudioClip MusicLose;

		[SerializeField] private AudioSource audioSource;

		[SerializeField] private float tweenDuration;

		[SerializeField] private double deltaTime, TIME;

		[SerializeField] private Transform swappingOverlay;

		[SerializeField] private bool ensureNoStartingMatches;

		public GameOverScreen GameOverSetup;
		public PauseMenu PauseMenuSetup;
		...
		private void Start()
		{
			...
			OnMatch += (type, count) => 
			{
				...
				if (type.name == "Parrot") {
					...
					ScoreCounter.Instance.Score += count * 2;
					deltaTime += 10.0;
				}
				else if (type.name == "Pig") {
					deltaTime -= 5.0;
					audioSource.PlayOneShot(loseSound);
				} else if (type.name == "Monkey"){
					deltaTime += 5.0;
				} else if (type.name == "Hippo"){
					ScoreCounter.Instance.Score += count;
					deltaTime -= 10.0;
					audioSource.PlayOneShot(loseSound);
				} else {
					ScoreCounter.Instance.Score += count;
		...
		void countDownPlaying(){
			audioSource.PlayOneShot(countDown);
			CancelInvoke("countDownPlaying");
		}
		...
		private void Update()
		{
			if (Input.GetKeyDown(KeyCode.Escape)){
				Time.timeScale = 0;
				PauseMenuSetup.SetPause();
			}
			
			if (ScoreCounter.Instance.Score > PlayerPrefs.GetInt("HScore", 0)){
				PlayerPrefs.SetInt("HScore", ScoreCounter.Instance.Score);
			}
			if ((TIME + deltaTime) - Time.timeSinceLevelLoad > 0) {
				if (isEndGame == false){
					if (1 <= (TIME + deltaTime) - Time.timeSinceLevelLoad && (TIME + deltaTime) - Time.timeSinceLevelLoad <= 12) Invoke("countDownPlaying", 1.0f);
				}
				TimeCounter.Instance.Time = (TIME + deltaTime) - Time.timeSinceLevelLoad;
			}
			else {
				TimeCounter.Instance.Time = 0;
				deltaTime = 0;
				if (isEndGame == false) {
					audioSource.Stop();
					audioSource.PlayOneShot(MusicLose);
					isEndGame = true;
				}
			
				GameOverSetup.Setup(ScoreCounter.Instance.Score);
			}
